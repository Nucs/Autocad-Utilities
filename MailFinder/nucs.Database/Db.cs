...
namespace nucs.Database {
    ...
    public static class Db {
        ...
        public static SqlConnection GetConnection() {
            ...
            try {
                if (db.State == ConnectionState.Closed)
                    db.Open();
            } catch (SqlException e) when (e.ToString().Contains("error: 40")) {
                goto _retry;
        ...
        public static SqlConnection GetConnection(string dbname) {
            ...
            var db = new SqlConnection(ConnectionStringSourceFormat.Replace("{catalog}", dbname));
            _retry:
            try {
                if (db.State == ConnectionState.Closed)
                    db.Open();
        ...
        /// <returns></returns>
        public static T Call<T>(Func<SqlConnection, T> sqlfunc, string db = null) {
            _retry:
            try {
        ...
        /// <param name="sqlfunc">Method that uses sql connection</param>
        /// <returns></returns>
        public static void CallForget(Action<SqlConnection> sqlfunc) {
            _retry:
            try {
