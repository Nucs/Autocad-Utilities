...
namespace Dapper {
    ...
    public static partial class SimpleCRUD {
        ...
        public static TKey Insert<TKey>(this IDbConnection connection, object entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) {
            ...
            BuildInsertParameters(entityToInsert, idProps[0].Name, sb);
            sb.Append(") values (");
            BuildInsertValues(entityToInsert, idProps[0].Name, sb);
            sb.Append(")");
            ...
            if ((keytype == typeof(int) || keytype == typeof(long)) && Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0) {
                sb.Append(";" + _getIdentitySql.Replace("@id", idProps[0].Name));
            } else {
                keyHasPredefinedValue = true;
            }
        ...
        private static void BuildSelect(StringBuilder sb, IEnumerable<PropertyInfo> props) {
            ...
            for (var i = 0; i < propertyInfos.Count(); i++) {
                if (propertyInfos.ElementAt(i).GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(IgnoreSelectAttribute).Name || attr.GetType().Name == typeof(NotMappedAttribute).Name))
                    continue;
        ...
        private static void BuildInsertValues(object entityToInsert,string idname, StringBuilder sb) {
            ...
            for (var i = 0; i < props.Count(); i++) {
                ...
                if (property.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(IgnoreInsertAttribute).Name))
                    continue;
                if (property.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(NotMappedAttribute).Name))
                    continue;
                if (property.GetCustomAttributes(true).Any(attr => attr.GetType().Name == typeof(ReadOnlyAttribute).Name && IsReadOnly(property)))
                    continue;

                if (property.Name.Equals(idname, StringComparison.OrdinalIgnoreCase) && property.GetCustomAttributes(true).All(attr => attr.GetType().Name != typeof(RequiredAttribute).Name) && property.PropertyType != typeof(Guid))
                    continue;
        ...
        private static void BuildInsertParameters(object entityToInsert, string idname, StringBuilder sb) {
            var props = GetScaffoldableProperties(entityToInsert).ToArray();
        ...
        private static bool IsEditable(PropertyInfo pi) {
            var attributes = pi.GetCustomAttributes(false).ToArray();
        ...
        private static string Encapsulate(string databaseword) {
            var capleft = _encapsulation[0];
            var capright = _encapsulation.Last();
            if (databaseword != null && databaseword.Length > 1 && ((databaseword[0] == capleft && databaseword[databaseword.Length - 1] == capright) || (databaseword[0] == '[' && databaseword[databaseword.Length - 1] == ']'))) //has been already provided capsulated.
                return databaseword.Replace('[', capleft).Replace(']', capright);
